...
func TestAllProcess(t *testing.T) {
	...
	testKeyManager2, _ := keys.NewPrivateKeyManager(priv2)//keys.NewKeyManager()
	testAccount2 := testKeyManager2.GetAddr()
	fmt.Printf("testAccount1: %v \n", testAccount1)
	fmt.Printf("testAccount2: %v \n", testAccount2)
	...
	//----- Get Markets  ------------
	// markets, err := client.GetMarkets(query.NewMarketsQuery().WithLimit(1000))
	// assert.NoError(t, err)
	// fmt.Printf("markets Num: %v \n", len(markets))
	//fmt.Printf("markets: %v \n", markets)
	//tradeSymbol := markets[0].TradeAsset
	// for _, m := range markets {
	// 	fmt.Printf("%v \n", m)
	// 	// if m.QuoteAsset == "BTC.B-918" {
	// 	//  	tradeSymbol = m.TradeAsset
	// 	//  	fmt.Printf("       %v \n", tradeSymbol)
	// 	// }		
	// }
	tradeSymbol := "BNB" 
	...
	depth, err := client.GetDepth(dq)
	assert.NoError(t, err)
	fmt.Printf("depth: %v \n", depth) 
	assert.True(t, depth.Height > 0)
	fmt.Printf("tradeSymbol: %v \n", tradeSymbol)
	...
	createOrderResult, err := client.CreateOrder(tradeSymbol, nativeSymbol, msg.OrderSide.BUY, 39385300, 100000000, true)
	assert.NoError(t, err)
